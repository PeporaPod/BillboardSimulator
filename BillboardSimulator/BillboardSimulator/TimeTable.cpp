...
TimeTable::TimeTable()
{
	linelist.Init();
	stringlist.Init();
	stringlist.Add_Multiple("SEG\\SegFile.enum");
}
...
//
//
void TimeTable::Set(LEDTABLE& ledtable, TABLESIZE tablesize, int id, int row, int x, int y, bool english_trigger)
{
	...
	for (int number = 0; number < row; number++) {
		...
		//路線色
		linelist.Set(ledtable, tablesize, timetable[id + number].line, 16 * number + y);
		//種別
		stringlist.Draw(ledtable, tablesize, timetable[id + number].type, x + 8, 16 * number + y, english_trigger);
		//発車時刻
		stringlist.SetDepartureTime(ledtable, timetable[id + number].time, x + 56, 16 * number + y);
		//行先
		stringlist.Draw(ledtable, tablesize, timetable[id + number].destination, x + 112, 16 * number + y, english_trigger);
	}
}
